...
```diff
import "github.com/mosheya/etcd/etcdserver"
...
```go
import "github.com/mosheya/etcd/embed"
...
Previously, `[endpoint]:[client-port]/health` returned manually marshaled JSON value. 3.3 now defines [`etcdhttp.Health`](https://godoc.org/github.com/mosheya/etcd/etcdserver/api/etcdhttp#Health) struct.
...
```go
import "github.com/mosheya/etcd/embed"
import "github.com/mosheya/etcd/etcdserver/api/v3rpc/rpctypes"
...
```go
import "github.com/mosheya/etcd/clientv3"
...
**If not specified, client-side send limit defaults to 2 MiB (1.5 MiB + gRPC overhead bytes) and receive limit to `math.MaxInt32`**. Please see [clientv3 godoc](https://godoc.org/github.com/mosheya/etcd/clientv3#Config) for more detail.
...
`clientv3` has deprecated `golang.org/x/net/context`. If a project vendors `golang.org/x/net/context` in other code (e.g. etcd generated protocol buffer code) and imports `github.com/mosheya/etcd/clientv3`, it requires Go 1.9+ to compile.
...
etcd now uses [`gcr.io/etcd-development/etcd`](https://gcr.io/etcd-development/etcd) as a primary container registry, and [`quay.io/mosheya/etcd`](https://quay.io/mosheya/etcd) as secondary.

Before

```bash
docker pull quay.io/mosheya/etcd:v3.2.5
```
